...
export default function StatusModal({ isOpen, onClose, callsign, status }: StatusModalProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-zinc-900/40 dark:bg-black/90 backdrop-blur-md">
        <motion.div 
          initial={{ opacity: 0, scale: 0.9, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.9, y: 20 }}
          className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-[1.25rem] w-full max-w-[280px] overflow-hidden shadow-2xl"
        >
          <div className="p-4 border-b border-black/5 dark:border-white/5 flex items-center justify-between bg-gradient-to-br from-zinc-200/50 dark:from-zinc-800/50 to-transparent">
            <div className="flex-1 min-w-0">
              <h2 className="text-2xl font-black text-zinc-900 dark:text-white font-mono tracking-tighter uppercase italic leading-none truncate">
            ...
            <button 
              onClick={onClose}
              className="p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-full text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-all shrink-0 ml-2"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
            ...
            <table className="w-full border-separate border-spacing-[2px]">
              <thead>
                <tr>
                  ...
                  {MODES.map(mode => (
                    <th key={mode} className="pb-1 text-[8px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600">
                      {mode}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {BANDS.map(band => (
                  <tr key={band}>
                    <td className="py-0.5 text-[9px] font-bold text-zinc-500 text-right pr-2 border-r border-black/5 dark:border-white/5">
                      {band}
                    </td>
                    {MODES.map(mode => {
                        ...
                        <td key={`${band}-${mode}`} className="p-0">
                          <div className={`
                            ...
                            ${s === 'confirmed' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 
                              ...
                              'bg-black/5 dark:bg-white/5 border-black/5 dark:border-white/5 text-zinc-900'}
                          `}>
          ...
          <div className="p-3 border-t border-black/5 dark:border-white/5 bg-black/5 dark:bg-black/20 flex items-center justify-center">
            <div className="flex gap-4">
              <div className="flex items-center gap-1 text-[8px] font-bold uppercase tracking-widest text-emerald-700 dark:text-emerald-500/80">
                <CheckCircle2 className="w-2.5 h-2.5" /> CFM
              </div>
              <div className="flex items-center gap-1 text-[8px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-500/80">
