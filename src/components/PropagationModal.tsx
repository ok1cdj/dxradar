...
export default function PropagationModal({ isOpen, onClose, data }: PropagationModalProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/40 dark:bg-black/80 backdrop-blur-sm">
        <motion.div 
          initial={{ opacity: 0, scale: 0.95, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 20 }}
          className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl"
        >
          ...
          <div className="p-6 space-y-8 max-h-[80vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center gap-2">
                <Sun className="w-8 h-8 text-amber-500" />
                <span className="text-xs font-bold uppercase tracking-widest text-zinc-500">Solar Flux Index</span>
                <span className="text-3xl font-black text-zinc-900 dark:text-white">{data.sfi}</span>
              </div>
              <div className="bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center gap-2 relative overflow-hidden">
                <Activity className="w-8 h-8 text-emerald-500" />
                <span className="text-xs font-bold uppercase tracking-widest text-zinc-500">Kp Index</span>
                <span className={`text-3xl font-black ${data.kp >= 4 ? 'text-rose-500' : 'text-zinc-900 dark:text-white'}`}>{data.kp}</span>
            ...
            {data.storm && data.storm.probability >= 50 && (
              <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center gap-4 animate-pulse">
                ...
                <div>
                  ...
                  <p className="text-xs text-rose-700 dark:text-rose-200/70">
            ...
            <div className="space-y-4">
              ...
              <div className="overflow-x-auto border border-black/5 dark:border-white/5 rounded-2xl custom-scrollbar">
                <table className="w-full text-left border-collapse min-w-[500px]">
                  <thead>
                    <tr className="bg-black/5 dark:bg-white/5 text-[10px] uppercase font-bold tracking-wider text-zinc-500">
                  ...
                  <tbody className="divide-y divide-black/5 dark:divide-white/5">
                    {bands.map(band => {
                      ...
                      return (
                        <tr key={band} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                          <td className="px-4 py-4 font-bold text-zinc-900 dark:text-white">{band}</td>
                          <td className="px-4 py-4 text-center font-mono text-zinc-700 dark:text-zinc-300">{status.val.toFixed(1)}</td>
                          ...
                          <td className="px-4 py-4 text-center font-mono text-zinc-700 dark:text-zinc-300">{status.forecast.toFixed(1)}</td>
                          <td className="px-4 py-4 text-center">
                            <span 
                              className="px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider"
          ...
          <div className="p-4 bg-black/5 dark:bg-black/40 border-t border-black/5 dark:border-white/5 flex justify-center">
